...
#define SET_GPIO_ALT(idx, alt)	{RESET_GPIO(idx); gpio[(idx/10)] |= (alt << (idx%10)*3);}
...
#define ALT_4			0x3	//0b011
#define ALT_5			0x1	//0b001

/* Initialize jtag pins using alterative function 4 */
void enable_jtag()
{
	SET_GPIO_ALT(JTAG_TRST, ALT_4);
	SET_GPIO_ALT(JTAG_TDI, ALT_4);
	SET_GPIO_ALT(JTAG_TMS, ALT_4);
	SET_GPIO_ALT(JTAG_TCK, ALT_4);
	SET_GPIO_ALT(JTAG_RTCK, ALT_4);
	SET_GPIO_ALT(JTAG_TDO, ALT_4);
}
